...
class Student(SQLModel, table=True):
    ...
    # [NEW FIX] Enforce unique roll numbers per institute
...
# --- NEW: User Profile & Security Routes ---
...
async def bulk_register(course_id: int, file: UploadFile = File(...), user: Instructor = Depends(get_current_user), session: Session = Depends(get_session)):
    ...
    for _, row in df.iterrows():
        try:
            ...
            if not roll or not name or not link_url:
                continue
        ...
        if not student:
            try:
                ...
                response = requests.get(link_url, headers=headers, timeout=15, allow_redirects=True)
                
                if response.status_code == 200:
                    content_type = response.headers.get('Content-Type', '')
                    if 'image' not in content_type and 'application/octet-stream' not in content_type:
                         errors.append(f"{roll}: Link returned a webpage, not an image.")
                         continue

                    img = decode_image_bytes(response.content)
                    if img is None: 
                        errors.append(f"{roll}: Invalid Image")
                        continue
                    ...
                    if encoding:
                        ...
                        errors.append(f"{roll}: No face found in image")
                        continue
                else:
                    errors.append(f"{roll}: Link not accessible (Status {response.status_code})")
                    continue
            except Exception:
                errors.append(f"{roll}: Network/Download Error")
                continue
...
async def download_excel(session_id: int, user: Instructor = Depends(get_current_user), session: Session = Depends(get_session)):
    ...
    # FIX: Sanitize filename to prevent errors with special chars like '/'
...
async def delete_student(student_id: int, user: Instructor = Depends(get_current_user), session: Session = Depends(get_session)):
    ...
    # 1. Delete Student-Course Links (Fixes the Foreign Key Crash)
    links = session.exec(select(StudentCourseLink).where(StudentCourseLink.student_id == student_id)).all()
    for link in links: 
        session.delete(link)
    
    # 2. OPTIONAL: Delete Attendance Logs for this student to keep DB clean
    # (If you want to keep history, remove these 3 lines)
...
async def mark_attendance(body: AttendanceRequest, session: Session = Depends(get_session)):
    ...
    if qr_created_at.tzinfo is None:
        qr_created_at = qr_created_at.replace(tzinfo=timezone.utc)
